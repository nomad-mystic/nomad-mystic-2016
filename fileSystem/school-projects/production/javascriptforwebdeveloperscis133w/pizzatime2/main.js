...
function Init() {
	...
	/*
	Notice that we are NOT using getElementById for the radio buttons -- getElementById doesn't work for radio buttons because there is more than one. We need to grab the entire group of radio buttons (remember -- the group is an array). The getElementsByName() function allows us to grab the entire array.

	In the same line of code, we assign the radio button group to a variable. Since the group is an array, the variable instantly becomes an array (just like when we assign a number to a variable, the variable becomes a number var).  Now, radSize is holding the entire radio button array.
	*/
	...
	document.getElementById("chkDelivery").onclick =
		function() {
			if (this.checked === true) {
				/* First of all, recall that *this* refers to the object/element that was the target of the event, i.e. the chkDelivery checkbox.  So we are asking if it's checked property is true, i.e. is it checked?
...
function PlaceOrder() {
	...
	// NEXT: Preparing to validate: clear all previous error msgs.  Keep in mind that it's possible that this isn't the first time we're running this function. The user may have clicked Place Order before and had errors.  So we have to start with a fresh validation each time.  Clear out all possible error msgs.
	g_sMsg = "";
	divErrors.innerHTML = "";
	...
	Using a multiple-choice IF, we find out which radio button they checked (if any), and then continue building our order msg.  Remember that ga_radSize[0] refers to the FIRST radio button, ga_radSize[1] refers to the SECOND, etc.
	...
	Comment out the IF statement above and UNcomment the one below.  Look at the IF statement above and notice how each branch has EXACTLY 
	the same code in it except for the index number of the radio button array.  That's a lot of redundancy.  And as you know by now, redundancy is bad.  
	Let's use a function to improve upon that. Peruse the IF statement below and when you feel comfortable with it, check out the AddSizeMsg() function.  
	Then get back to me.
	*/
	...
	*/
	...
	Notice the SEPARATE, INDIVIDUAL IF statements, because (unlike radio buttons), the user can select more than one.  Also, we are not requiring them to 
	select a topping.
	*/
	// if (ga_chkTopping[0].checked === true) {
	// 	g_sMsg += ga_chkTopping[0].value + "<br>";
	// }
	// // NOTICE THE BLANK LINE BETWEEN EACH IF STATEMENT -- this reinforces to anyone reading the code (including ourselves) that these are **SEPARATE** IF 
	// // statements
	...
	Answer: If we write directly to divOrder.innerHTML, we run into a problem if the user has any errors.  Suppose they check the Delivery 
	checkbox but neglect to put in an address.  We give them an error msg, which is great.  But, we've already written a partial order to the Order div.  Tacky.
	To keep that from happening, we build our string to a variable instead.  If they make it thru without any errors, only then do we write from our global 
	var to divOrder.
	*/
	...
	*/
	...
	The selectedIndex property of a select dropdown element gives us the index number of which item in that array is currently selected.  
	The first item has a selectedIndex of 0, the second item has a selectedIndex of 1, etc.
	*/
